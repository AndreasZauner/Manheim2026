(function () {
  ...
  const layers = {
    radar: {
      ...
      legend: 'Farbskala: st&auml;rkere Farben bedeuten st&auml;rkeren Niederschlag.'
    },
    warnings: {
    ...
    wind: {
      ...
      opacity: 0.8,
      title: 'DWD Wind 10 m',
      legend: 'Windfahnen zeigen Richtung und St&auml;rke.'
    },
    uv: {
      layer: 'dwd:UVIndex',
      styles: 'uvindex',
  ...
  async function initInteractiveMap(frame, key, signature) {
    ...
    try {
      ...
      const leafletMap = window.L.map(canvas, {
        zoomControl: true,
      ...
      window.L.circleMarker([PLACE.lat, PLACE.lon], {
        radius: 6,
        color: '#0f2740',
        weight: 2,
        fillColor: '#2e7d32',
        fillOpacity: 0.95
      }).addTo(leafletMap).bindTooltip(PLACE.label, { direction: 'top' });
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-current{
        grid-template-columns:auto minmax(92px,1.15fr) repeat(3,minmax(76px,.8fr));
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-current strong{
        display:block;
        font-size:1rem;
        line-height:1.1;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-current small{
        display:block;
        line-height:1.2;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-days{
        grid-template-columns:repeat(7,minmax(82px,1fr));
        overflow-x:auto;
        padding-bottom:2px;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day{
        min-width:82px;
        padding:7px;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day-top{
        gap:4px;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day-top span{
        font-size:1.05rem;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day strong{
        font-size:.68rem;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day b{
        font-size:.86rem;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day small,
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-day em{
        white-space:normal;
        line-height:1.15;
        font-size:.66rem;
      }
      #leitstandWeatherModule.has-weather-map-fix .weather-v2-map-frame.has-interactive-map{
        height:184px;
      ...
      #leitstandWeatherModule.is-weather-max.has-weather-map-fix .weather-v2-map-frame.has-interactive-map{
        height:390px;
      }
      .weather-map-fix-canvas{
        width:100%;
        height:100%;
        font-family:inherit;
      }
      .weather-map-fix-legend{
        position:absolute;
        left:10px;
        bottom:10px;
        z-index:600;
        ...
        gap:4px;
        max-width:172px;
        border:1px solid rgba(207,221,235,.95);
        border-radius:8px;
        background:rgba(255,255,255,.94);
        padding:7px;
        box-shadow:0 8px 18px rgba(15,39,64,.12);
      }
      .weather-map-fix-legend strong{
        font-size:.7rem;
        color:var(--text);
      }
      .weather-map-fix-legend span{
        font-size:.63rem;
        line-height:1.18;
        color:var(--muted);
      }
      .weather-map-fix-legend img{
        display:block;
        max-width:100%;
        max-height:68px;
        object-fit:contain;
      }
      .weather-map-fix-legend img.is-hidden{
        display:none;
      }
      .weather-map-fix-error{
        position:absolute;
